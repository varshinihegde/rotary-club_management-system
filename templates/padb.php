...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action == 'approve') {
            ...
            // Move to members table and delete from pending
            $stmt = $conn->prepare("INSERT INTO members (member_id, m_name, m_email, m_phone_no, join_date, address, m_password)
                                  SELECT CONCAT('MEM', LPAD(FLOOR(RAND() * 100000), 5, '0')), p_name, p_email, p_phone, NOW(), '', ? 
                                  FROM pending_approval WHERE pending_id = ?");
            $stmt->bind_param("ss", $password, $application_id);
            $stmt->execute();
                        ...
                        <tbody>
                            <?php if ($result && $result->num_rows > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php 
                                        // Check if achievements column contains a file path or BLOB data
                                        if (!empty($row['achievements'])) {
                                            // If it's a path (starts with uploads/)
                                            if (strpos($row['achievements'], 'uploads/') === 0) {
                                                $file_path = $row['achievements'];
                                                echo '<a href="'.$file_path.'" target="_blank" class="btn btn-sm btn-info">
                                                    <i class="fas fa-file-pdf"></i> View Document
                                                </a>';
                                            } 
                                            // If it's BLOB data (binary)
                                            else {
                                                // Generate a temporary file to view the BLOB
